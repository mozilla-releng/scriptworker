...
@pytest.mark.asyncio
async def test_mocker_run_tasks_caught_exception(
    context, successful_queue, mocker, func_to_raise, exc, expected
):
...
@pytest.mark.asyncio
async def test_mocker_upload_artifacts_uncaught_exception(
    context, successful_queue, mocker
):
...
@pytest.mark.asyncio
async def test_unexpected_exception_catch_and_tell_taskcluster(
    context, successful_queue, mocker
):
    def fail():
        raise Exception()
...
async def test_run_tasks_no_cancel(context, mocker):
    mocker.patch(
        "scriptworker.worker.claim_work", create_async(_MOCK_CLAIM_WORK_RETURN)
    )
    ...
    mocker.patch(
        "scriptworker.worker.filepaths_in_dir", create_sync(["one", "public/two"])
    )
...
async def test_run_tasks_cancel_sleep(context, mocker):
    ...
    mocker.patch(
        "scriptworker.worker.claim_work", create_async(_MOCK_CLAIM_WORK_NONE_RETURN)
    )
...
async def test_run_tasks_cancel_cot(context, mocker):
    ...
    mock_do_upload.assert_called_once_with(
        context, ["public/logs/chain_of_trust.log", "public/logs/live_backing.log"]
    )
