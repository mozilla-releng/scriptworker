...
ENV_CREDS_PARAMS = (
    ({"TASKCLUSTER_ACCESS_TOKEN": "x", "TASKCLUSTER_CLIENT_ID": "y"}, {"accessToken": "x", "clientId": "y"}),
    (
        {"TASKCLUSTER_ACCESS_TOKEN": "x", "TASKCLUSTER_CLIENT_ID": "y", "TASKCLUSTER_CERTIFICATE": "z"},
        {"accessToken": "x", "clientId": "y", "certificate": "z"},
    ),
)
...
def _fill_missing_values(config):
    return {key: "filled_in" if value == "..." else value for key, value in config.items()}
...
@pytest.mark.parametrize("args", (["1", "2", "3", "4"], ["x", os.path.join(os.path.dirname(__file__), "data", "bad.json")], []))
...
def test_apply_product_config():
    ...
    in the config.
    """
    c = {"cot_product": "thunderbird", "unkeyed": "no keys", "keyed": {"by-cot-product": {"thunderbird": "expected", "firefox": "unexpected"}}}
    expected_config = {"cot_product": "thunderbird", "unkeyed": "no keys", "keyed": "expected"}
