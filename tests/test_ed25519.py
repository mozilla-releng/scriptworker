...
from . import fake_session, fake_session_500, rw_context, successful_queue, touch
...
def test_bad_file():
    with pytest.raises(ScriptWorkerEd25519Error):
        swed25519.ed25519_private_key_from_file(os.path.join(ED25519_DIR, "foo.json"))
...
@pytest.mark.parametrize(
    "unsigned_path, signature_path, public_key_path, raises",
    (
        (
            # Good
...
@pytest.mark.parametrize(
    "pubkey_path, file_path, sig_path, exception",
    (
        (
            os.path.join(ED25519_DIR, "scriptworker_public_key"),
            os.path.join(ED25519_DIR, "foo.json"),
        ...
        (None, os.path.join(ED25519_DIR, "foo.json"), os.path.join(ED25519_DIR, "foo.json.scriptworker.sig"), ScriptWorkerEd25519Error),
    ),
)
