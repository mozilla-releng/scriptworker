...
def context(rw_context):
    ...
    rw_context.config["task_max_timeout"] = 0.1
...
@pytest.mark.parametrize(
    "filename, original_content, expected_content_type, expected_encoding",
    (
...
async def test_download_artifacts(context):
    ...
    expected_urls = ["https://queue.taskcluster.net/v1/task/dependency1/artifacts/foo/bar", "https://queue.taskcluster.net/v1/task/dependency2/artifacts/baz"]
    expected_paths = [os.path.join(context.config["work_dir"], "foo", "bar"), os.path.join(context.config["work_dir"], "baz")]
...
async def test_download_artifacts_timeout(context):
    ...
    urls = ["https://queue.taskcluster.net/v1/task/dependency1/artifacts/foo/bar"]
    expected_paths = [os.path.join(context.config["work_dir"], "foo", "bar")]
...
def test_get_upstream_artifacts_full_paths_per_task_id(context):
    artifacts_to_succeed = [
        ...
        {"paths": ["public/file_b1", "public/file_b2"], "taskId": "dependency2", "taskType": "signing"},
        {"paths": ["some_other_folder/file_c"], "taskId": "dependency3", "taskType": "signing"},
        {
    ...
    context.task["payload"] = {
        "upstreamArtifacts": [
            {"paths": ["public/failed_optional_file1"], "taskId": "failedDependency1", "taskType": "signing", "optional": True},
            {"paths": ["public/failed_optional_file2", "public/failed_optional_file3"], "taskId": "failedDependency2", "taskType": "signing", "optional": True},
        ]
    }
    ...
    assert succeeded_artifacts == {
        ...
        "dependency3": [os.path.join(context.config["work_dir"], "cot", "dependency3", "some_other_folder", "file_c")],
    }
...
def test_get_and_check_single_upstream_artifact_full_path(context):
    ...
    assert get_and_check_single_upstream_artifact_full_path(context, "dependency1", "public/file_a") == os.path.join(
...
def test_get_single_upstream_artifact_full_path(context):
    ...
    assert get_single_upstream_artifact_full_path(context, "dependency1", "public/non_existing_file") == os.path.join(
        context.config["work_dir"], "cot", "dependency1", "public", "non_existing_file"
    )

    assert get_single_upstream_artifact_full_path(context, "non-existing-dep", "public/file_a") == os.path.join(
        context.config["work_dir"], "cot", "non-existing-dep", "public", "file_a"
    )
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ([{"taskId": "someTaskId", "paths": ["optional_artifact_1"], "optional": True}], {"someTaskId": ["optional_artifact_1"]}),
        (
...
# assert_is_parent {{{1
@pytest.mark.parametrize("path, parent_path, raises", (("/foo/bar/baz", "/foo/bar", False), ("/foo", "/foo/bar", True), ("/foo/bar/..", "/foo/bar", True)))
