...
def verify_task_in_task_graph(task_link, graph_defn, level=logging.CRITICAL):
    ...
    """
    ignore_keys = ("created", "deadline", "dependencies", "expires", "projectId", "schedulerId", "taskQueueId", "priority")
    errors = []
    ...
    # resolve timestamps relative to the task creation time
    if sys.version_info >= (3, 11):
        # strip the timezone, assuming UTC (resolve_timestamps currently expects a naive datetime object)
        created = datetime.datetime.fromisoformat(runtime_defn["created"]).replace(tzinfo=None)
    else:
        # before 3.11, fromisoformat doesn't support `YYYY-mm-ddTHH:MM:SS.SSSZ` as generated by taskgraph, so strip the Z for parsing
        assert runtime_defn["created"].endswith("Z")
        created = datetime.datetime.fromisoformat(runtime_defn["created"][:-1])
