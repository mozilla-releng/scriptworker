...
def ed25519_private_key_to_string(key):
    ...
    return base64.b64encode(
        key.private_bytes(encoding=serialization.Encoding.Raw, format=serialization.PrivateFormat.Raw, encryption_algorithm=serialization.NoEncryption()), None
    ).decode("utf-8")
...
default to ``config['ed25519_public_keys']``."""
    ...
    for key_type, seeds in pubkeys.items():
        for seed in seeds:
            try:
                verify_ed25519_signature(ed25519_public_key_from_string(seed), contents, signature, "didn't work with {}".format(seed))
                log.info("Verified good with {} seed {} !".format(key_type, seed))
                sys.exit(0)
