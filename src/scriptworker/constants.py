...
DEFAULT_CONFIG: immutabledict[str, Any] = immutabledict(
    {
        ...
        # XXX While migrating Firefox from hg.mozilla.org to GitHub, graphs are
        # still generated by decision tasks from hg, but some tasks in them
        # clone from the GitHub mirror instead. This maps an hg repo to its
        # GitHub mirror. A task whose source is the mirror of its parent task's
        # repo is treated as coming from the parent's repo; its definition has
        # already been verified against the parent task's graph. Parent
        # (decision/action) tasks themselves are never mapped.
        # Remove this once the migration is complete.
        "hg_git_mirrors": immutabledict(
            {
                "by-cot-product": immutabledict(
                    {
                        "adhoc": immutabledict({}),
                        ...
                        "firefox": immutabledict(
                            {
                                "/mozilla-central": "/mozilla-firefox/firefox",
                                "/integration/autoland": "/mozilla-firefox/firefox",
                                "/releases/mozilla-beta": "/mozilla-firefox/firefox",
                                "/releases/mozilla-release": "/mozilla-firefox/firefox",
                                "/releases/mozilla-esr115": "/mozilla-firefox/firefox",
                                "/releases/mozilla-esr140": "/mozilla-firefox/firefox",
                                "/releases/mozilla-esr153": "/mozilla-firefox/firefox",
                            }
                        ),
                        "glean": immutabledict({}),
